...
<span class="mr-2 d-none d-lg-inline text-gray-600 small">{{ Auth::user()->name }}</span>
  ...
  <div>
    ...
    <div class="resultado">
        @if(isset($totalVisitas))
          Total de Visitas: {{ $totalVisitas }}
        @else
          Visitas a la App: {{ $appVisitas }} || Visitas a la Landing Page: {{ $landingPageVisitas }}
        @endif
      </div>      

    <form action="{{ route('filtrar.visitas') }}" method="GET" class="container mb-4">
      <div class="row g-2 justify-content-center">
        <div class="col-md-2">
          <input type="number" name="dia" class="form-control" placeholder="Día">
        </div>
        <div class="col-md-2">
          <input type="number" name="mes" class="form-control" placeholder="Mes">
        </div>
        <div class="col-md-2">
          <input type="number" name="anio" class="form-control" placeholder="Año">
        </div>
        <div class="col-md-3">
          <select name="pagina" class="form-select">
        ...
        <div class="col-md-2">
          <button type="submit" class="btn btn-success" style="background-color:rgb(100, 0, 109);">Buscar</button>
        </div>
      </div>
    </form>
  ...
  <script>
    const visitasData = {
      labels: ['Landing Page', 'Aplicación'],
      datasets: [{
        label: 'Número de Visitas',
        data: [{{ $landingPageVisitas }}, {{ $appVisitas }}],
        backgroundColor: [' #9f44d3', 'rgb(47, 0, 75)'],
        borderColor: [' #9f44d3', 'rgb(30, 0, 48)'],
        borderWidth: 1
      }]
    };
